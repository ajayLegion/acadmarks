  ...
  const submit = () => {
    ...
    update(d => {
      const duplicate = d.students.some(
        s => s.SRN?.toUpperCase() === form.SRN.trim().toUpperCase() && s.id !== editing
      );
      ...
      if (editing) {
        d.students = d.students.map(s =>
          s.id === editing ? { ...s, ...student, courses: s.courses || [] } : s
        );
        notify("Student updated");
      } else {
        d.students.push({ id: uid(), ...student, courses: [], riskLevel: "SAFE" });
    ...
    <div>
      ...
      <div className="two-col">
        <div className="card" style={{ position: "sticky", top: 80 }}>
          <div className="card-title">{editing ? "Edit Student" : "Add Student"}</div>
        ...
        <div className="card">
          ...
          {filtered.length ? (
              ...
              <table>
                ...
                <tbody>
                  {filtered.map(student => {
                    ...
                    return (
                      <tr key={student.id} style={{ background: risk ? "#fffbeb" : "transparent" }}>
                        <td>{student.classSection || "-"}</td>
                        <td><code>{student.SRN}</code></td>
                        <td style={{ fontWeight: 600, color: "var(--text-h)" }}>{student.name}</td>
                        <td>{student.department || "-"}</td>
                        <td>{student.semester || "-"}</td>
                        <td style={{ fontWeight: 800 }}>{courses.length}</td>
                        <td style={{ fontWeight: 800, color: lowIA ? "#ef4444" : "#22c55e" }}>
                          {lowIA}
                        </td>
                        <td>
                          <span className={load.ok ? "chip chip-pass" : "chip chip-risk"}>
                            {load.label}
                          </span>
                        </td>
                        <td>
                          <div style={{ display: "flex", gap: 5 }}>
                            <button className="btn btn-secondary btn-sm" onClick={() => edit(student)}>
                              Edit
                            </button>
                            <button className="btn btn-danger btn-sm" onClick={() => del(student.id)}>
                              Delete
                            </button>
                          </div>
                        </td>
            ...
            <EmptyState msg={search ? "No students match your search" : "No students found"} />
          )}
        </div>
      </div>
    </div>
